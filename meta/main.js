...
function renderCommitInfo(data, commits) {
  // Create the dl element
    ...
    // Add total LOC
    const totalLOC = d3.sum(commits.flatMap(d => d.lines), d => d.length); // or data.length if each row counts as 1
    ...
    const numFiles = d3.groups(commits.flatMap(d => d.lines), d => d.file).length;
    addStat('Files', numFiles);

    const numDays = d3.groups(commits, d => d.date.toDateString()).length;
    ...
    addStat('Average File Length', d3.format('.2f')(averageFileLength));

    const longestLine = d3.greatest(commits.flatMap(d => d.lines), d=> d.length);
...
function onStepEnter(response) {
  const currentDate = response.element.__data__.datetime;

  const filtered = commits.filter(d => d.datetime <= currentDate);
